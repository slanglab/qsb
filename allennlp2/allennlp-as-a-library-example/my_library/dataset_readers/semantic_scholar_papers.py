    ...
    def _read(self, file_path):
        with open(cached_path(file_path), "r") as data_file:
            ...
            for line in data_file:
                ...
                title = paper_json['sentence']
                venue = paper_json['label']
                yield self.text_to_instance(title, venue)

    @overrides
    def text_to_instance(self, title: str, venue: str = None) -> Instance:  # type: ignore
        # pylint: disable=arguments-differ
        tokenized_title = self._tokenizer.tokenize(title)
        title_field = TextField(tokenized_title, self._token_indexers)
        # abstract is not used in model. just for ez coding
        fields = {'title': title_field, 'abstract': title_field}
