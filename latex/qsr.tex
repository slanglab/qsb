...
We define one unit of computational cost to be the computation required to evaluate one subtree for possible pruning; i.e. the cost of computing $p(y=1 | \bm{x})$. In the worst-case, our method will prune one singleton subtree (consisting of only one vertex) at each of the $N$ timesteps during compression. (Starting from the leaves and proceeding to the root of the original dependency parse). If a dependency parse contains $V$ original vertexes, and each remaining vertex is evaluated for possible deletion at each timestep, then the iterative deletion method requires at most ${\sum_{i = 0}^N V - i = O(V^2)}$ possible operations. 


\subsection{Empirical costs of transition-based compression}\label{s:empiricalcost}
However, we analyze the empirical costs of our algorithm and find that it has much smaller computational requirements. The quadratic worst-case overestimates our algorithm's costs.
