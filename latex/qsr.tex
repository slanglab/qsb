...
\textbf{State change} &             \textbf{Definition}                                                    &      \textbf{Description}    \\ \hline
Start      & START $\Rightarrow ( V=\emptyset,  B=[v_1, v_2 ... v_n])$ & Initialize the buffer with $v \in V_s$, arranged breadth-first \\ \hline
\textsc{Prune}              & $(V, [v|B]), v \in V,  \Rightarrow (V \setminus  T(v), B)$ & Remove the subtree rooted at $v$ in $s$ from $V$ \\  \hline
$\textsc{Insert}$             & $(V, [v|B]), v \notin V, \Rightarrow (V \cup T(v), B)$ & Insert the subtree rooted at $v$ in $s$ into $V$  \\ \hline
Not \textsc{Prune}           & $(V, [v|B]), v \in V, \Rightarrow (V, B)$ & Don't remove the subtree  \\ \hline
Not \textsc{Insert}          &       $(V, [v|B]), v \notin V, \Rightarrow (V, B)$ &   Don't insert the subtree    \\ \hline
Stop             & $ (V, B=[]) \Rightarrow$ STOP & Compression ends when the buffer is empty \\                                               
\end{tabular}
\caption{A transition-based sentence compression system with a \textsc{Prune} and \textsc{Insert} operation. The state of the compression system is a tuple $(V, B)$, where $B$ is an ordered buffer of tokens and $[v|B]$ indicates that $v$ is at the head of the buffer. $V$ is a subset of vertexes from the original sentence, $s$. $T(v)$ denotes the subtree rooted at $v$ in the original sentence. The prune operation removes all tokens in $T(v)$ from $V_c$. The insert operation adds all vertexes from $T(v)$ into $V_c$. If the compression system does not \textsc{Prune} or does not \textsc{Insert}, $v$ is simply popped from the top of the buffer. $B$ is initialized with all tokens from $s$ (arranged in breadth-first order) to start compression. Once the buffer is empty, compression stops and all vertexes in $V$ are linearized in their original order. These operations can fully reconstruct all shortenings in a standard compression corpus \cite{filippova2013overcoming}.}
\end{table*}
