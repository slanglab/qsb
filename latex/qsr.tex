...
\item{The symbol $\langle o \cdot d \rangle$ is formed by concatenating two different markers: (1) a start symbol of type $o$, indicating the beginning of the sequence of tokens affected by the operation $o$, and (2) a marker indicating $d$, the dependency type governing $T(v)$. The symbol $\langle / o \cdot d \rangle$ is formed identically, only the marker indicates the end of the sequence.\footnote{BracketTags allow us to encode some aspects of syntax trees using a vanilla LSTM \cite{Vinyals2015GrammarAA,Aharoni2017TowardsSN}, without the added complexities of explicitly encoding nested structures into the architecture of the network \cite{Tai2015ImprovedSR,Dyer2016RecurrentNN}.}}
\end{itemize}
...
\textbf{Architecture and training} Following recent work on LSTM classification for sentence-level tasks \cite{D17-1070}, we predict the binary, oracle operation using a Bi-LSTM layer, a max pooling layer, an affine layer and a non-linearity. We maintain two separate affine and non-linear layers: one for the \textsc{Prune} operation and one for the \textsc{Insert} operation. Figure \ref{f:example} shows the architecture of the network.
