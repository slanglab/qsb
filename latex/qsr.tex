...
\textsc{vertex addition} & \small 0.881 & {\small 0.745}& \small 5.2 \\
\textsc{vertex addition}$_{NN}$& {\small 0.884}& {\small 0.731}& {\small 480.9} \\
\end{tabular}
\caption{Test results for constrained compression. $^*$Latency is shown in milliseconds (ms.) per sentence. \textsc{vertex addition} achieves higher F1 with \speedup lower latency than the \textsc{ilp}. \textsc{vertex addition}$_{NN}$ achieves highest F1, but runs slowly on a CPU. All differences are significant {\tiny $(p < 10^{-2})$}.}
\label{t:results}
\end{table}

\section{Conclusion}

Low-latency, query-focused compression is important for search user interfaces. We introduce a new \textsc{vertex addition} technique for such settings, which is \speedup faster than an ILP baseline while achieving higher F1 scores. 
