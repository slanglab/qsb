...
\caption{A new search interface (boxed, top) returns a snippet summary consisting of three compressions which must contain a users' query $Q$ (bold) and must not exceed $b=$75 characters in length. The source sentence $S$ for the final compression $C$ is shown below (\textit{italics}).}
\label{f:qf}
\end{figure}
