...
\begin{abstract}
Traditional approaches to extractive sentence compression seek to reduce the length of a sentence, while retaining the most ``important'' information from the source. But query-focused applications such as document search engines or exploratory search interfaces place additional lexical and length requirements on compression systems. This study examines extractive sentence compression under  constraints, using both classical ILP-based methods and modern neural network techniques.
\end{abstract}

\section{Introduction}
Traditional approaches within natural language processing view textual summarization as a sentence selection problem. Traditionally, the task of extractive sentence compression seeks to further shorten such summaries by deleting ``unimportant" words. However, effective sentence compression is also crucial in user-facing search systems, such as traditional search engines, concept map browsers and new forms of exploratory textual interfaces. \ahcomment{Cite hearst, IR book, falke, marchioni. examples.} 

These settings violate the traditional assumptions of the sentence compression problem. In place of the traditional goal of retaining the most ``important'' from source sentences for display in a summary, in such query-focused settings compression systems must instead $(i)$ contain a users' query term $(ii)$ remain readable or coherent and  $(iii)$ fit in the space allotted on screen.
