...
We define a markup function $m(V,v)=\bm{x}$, which ``shows'' the LSTM what a compressed sentence would ``look'' like if the subtree headed at $v$ were to be inserted into or pruned from the compression. More precisely, as before (\S\ref{s:system}), $V$ is a subset of vertexes from the original sentence $s$, and $v$ is some vertex from $s$. $T(v)$ denotes the subtree rooted at $v$ in the original sentence. \textbf{the markup} $\bm{x}$ is the symbol sequence [SOS, $\ell(V_L)$, BracketTag, $\ell(T(v))$, BracketTag, $\ell(V_R)$, EOS], where

\begin{itemize}
\item{The symbols SOS and EOS are special markers denoting the start and end of $\bm{x}$}
\item{$V_L \subseteq V$ is all  vertexes from $V$ which occur to the left of $T(v)$ in $s$, linearized in their original order.}
\item{$\ell(V_R) \subseteq V$ is identical to $\ell(V_L)$, except that vertexes in $V_R$ must occur to the right of $T(v)$).}
...
\footnote{For \textsc{Prune} operations, it will be the case that $T(v) \subseteq V$. For \textsc{Insert} operations, it will be the case that $T(v) \nsubseteq V$. For both operations, $T(v) \cap V_R = \emptyset = T(v) \cap V_L $.} 

or instance, in Figure \ref{f:example}, \ahcomment{TODO fix once figure redone} the BracketTag \textit{$<$tree$>$--\textsc{Insert}--nmod} indicates that (1) $T(v)$ is governed by an \rdep{nmod} relation, (2) the proposed operation is \textsc{Insert} and (3) the marker symbol comes at the start of $T(v)$ in $\bm{x}$. 
