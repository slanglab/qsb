...
\end{tabular}
\caption{Test results for constrained compression. $^*$Latency is shown in milliseconds (ms.) per sentence. \textsc{vertex addition} achieves higher F1 than the \textsc{ilp} with \speedup lower latency; all differences between the two methods are significant {\tiny $(p < .01)$}. The neural method {\textsc{vertex addition}$_{NN}$} achieves highest F1, but runs slowly on a CPU.}
\label{t:results}
\end{table}
