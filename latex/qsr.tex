...
Existing compression techniques are poorly suited to this task. While methods based on integer linear programming (ILP) can trivially accommodate such length and lexical restrictions \cite{clarke2008global,filippova2013overcoming,Wang2017CanSH}, these approaches rely on slow third-party solvers to optimize an NP-hard integer linear programming objective \cite{clarke2008global}. Meanwhile, newer LSTM tagging methods \cite{filippova2015sentence} do not allow practitioners to specify length or lexical constraints, and require expensive graphics processing units (GPUs) to achieve low runtime latency. These deficits prevent application of existing compression techniques in search user interfaces \cite{marchionini2006exploratory,hearst2009search,falke2017graphdocexplore}, where length, lexical and latency requirements are paramount. 

We thus present a new stateful method for query-focused compression, which grows a forest in a sentence's dependency parse. This approach is theoretically and empirically faster than ILP-based techniques, and achieves higher F1 scores.
...
We also experiment with learning acceptance decisions with a standard LSTM classifier, using a common max-pooling architecture and standard neural framework \cite{D17-1070,Gardner2017AllenNLP}. Our stateful procedure divides $V$ into four subsets, $P, C, \neg C$, and $\{v_i\}$. During training, we learn four embeddings for each word $w$ in the vocabulary: one for each subset which might contain $w$ during compression. We employ use corresponding embeddings based on state of the compression system at test time.
...
%\footnote{Longer sentences are always less probable than shorter sentences; rarer words make a sequence less probable.} 
...
In search applications, such latency gains are non-trivial: real users are measurably hindered by interface lags \cite{Nielsen,heerschei,Liu2014TheEO}.
