...
We assign an oracle operation to each $v \in V_s$, by walking the original dependency parse, breadth-first. For roughly two thirds of vertexes in the breath-first walk of ${(V_s,E_s)}$, the oracle does not execute a $\textsc{Prune}$ or $\textsc{Insert}$ operation. In these cases, we say that the oracle executes a $\textsc{Nop}$, which oracle leaves ${(V_c,E_c)}$ unmodified.
