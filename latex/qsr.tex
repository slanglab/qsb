...
In this work, we define a neural, transition-based sentence compression system, inspired by other kinds of transition-based models within NLP \cite{nivre2003}. 
\ahcomment{Now citing Nivre 2003, found via Chris Dyer papers. I am trying to find other areas (besides parsing) where transition-based models have been influential in NLP. I have found very old or very early or rarely cited things from workshops... but don't seem to be finding much else. I am searching,  "transition-based" -parsing}
Let ${(V_s,E_s)}$ denote the original, unchanging dependency parse of a sentence. At each timestep during compression, our system has a state ${(V_c,E_c)}$, which is some subgraph of ${(V_s,E_s)}$. The compression ${(V_c,E_c)}$ may or may not be fully-connected. 
...
We identified $\textsc{Prune}$ and $\textsc{Insert}$ via empirical experiment: we found that for all compressions in a large, standard  corpus \cite{filippova2013overcoming} there exists an oracle path of at most $|V_s|$ $\textsc{Prune}$ and $\textsc{Insert}$ operations which can fully reconstruct the shortened sentence. 
...
Because  is only possible to $\textsc{Prune}$ $T(v)$ if $v$ is included in $V$ (and only possible to \textsc{Insert} $T(v)$ if $v$ is not in $V$), it is always clear when a compression system should call $m_{\textsc{I}}$ or $m_{\textsc{P}}$. 
