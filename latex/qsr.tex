...
After executing a string of $\textsc{Prune}$ and $\textsc{Insert}$ operations, all tokens in ${(V_c,E_c)}$ are linearized in their original order to return a shortened sentence.
...
Only some operations are valid for some configurations of the state. If some vertex $v \in V_s$ is also included in the compression, then $\textsc{Insert}(v)$ is not valid as $v$ is already present in the compression. Similarly, if some vertex $v^{\prime} \notin V_c$ is not included in the compression, then $\textsc{Prune}(v^{\prime})$ is not valid as $v^{\prime}$ cannot be pruned.

Because all $v \in V_s$ are either in $V_c$ or not in $V_c$, for any given vertex--state pair either the operation \textsc{Prune} is valid or the operation \textsc{Extract} is valid, but not both. We can therefore refine the $\textsc{Nop}$ operation. If the oracle does not prune a given subtree, we say that the oracle executes $\textsc{Nop}_P$; if the oracle does not insert a given subtree, we say that the oracle executes $\textsc{Nop}_I$.

Following \cite{D14-1082}, we then create a training set of tuples: ${((V_c,E_c)_j, y_j)}$, where $(V_c,E_c)_j$ is the state of the system at step $s$ along the oracle path and $y_j$ is the oracle operation $\in  \{ \textsc{Prune}(v_c), \textsc{Insert}(v_s), \textsc{Nop}_P, \textsc{Nop}_I \}$.

We then train a standard LSTM classifier to predict the oracle operation given the state. 
