...
We use a large corpus of sentence--compression pairs \cite{filippova2013overcoming} to define a set of 8 million training instances $(\bm{x}_j, y_j)$, where $\bm{x}_j$ is the markup at step $j$ of some oracle path for some sentence $s$ in the training set, and $y_j \in \{0,1\}$ is the binary oracle choice at timestep $j$.
...
In future work, we plan to examine bottom-up compression methods, which start from the leaves of the original sentence and proceed to the root. We hypothesize that such methods might require a smaller number of local decisions about adding (or not adding) vertexes as compression proceeds, leading to greater computational efficiency.

Finally, some shortened sentences will modify the meaning of a sentence. Identifying such cases is a special case of the unsolved textual entailment problem \cite{snli_bowman,Pavlick2016SoCalledNA,linzencompression}. We hope to apply evolving research in this area to sentence compression in the future.  
