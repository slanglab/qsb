...
\caption{A new search interface (boxed, top) returns a snippet consisting of three compressions which must contain a users' query $Q$ (bold) and must not exceed $b=$75 characters in length. We show the source sentence $S$ (italics, bottom) for the final compression $C$.}
\label{f:qf}
\end{figure}
