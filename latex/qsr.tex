...
We define a markup function $m(V,v)=\bm{x}$, which maps a subset of vertexes from the original sentence $V$, along with some vertex from the original sentence $v$ to a sequence of symbols $\bm{x}$. The markup function, ``shows'' the LSTM what a compressed sentence would ``look'' like if the subtree headed at $v$ were to be inserted into or pruned from the compression. \textbf{The markup} $\bm{x}$ is the symbol sequence [SOS, $\ell(V_L)$, BracketTag, $\ell(T(v))$, BracketTag, $\ell(V_R)$, EOS], where
...
For instance, in Figure \ref{f:example}, \ahcomment{TODO fix once figure redone} the BracketTag \textit{$<$tree$>$--\textsc{Insert}--nmod} indicates that (1) $T(v)$ is governed by an \rdep{nmod} relation, (2) the proposed operation is \textsc{Insert} and (3) the marker symbol comes at the start of $T(v)$ in $\bm{x}$. 
