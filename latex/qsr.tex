...
\item todo p-values
\item update markup
\item Note: these Q and r are going to be well-behaved! I.e. there is usually a good compression here. Should be revisited.
\item Low priority, but would be interesting to see what the penalty is for prune only methods. What is the F1 cost of not doing insert?
\end{enumerate}
}
...
\ahcomment{TODO: describe assessing convergence w/ epoch level F1}
...
Iterative deletion is quadratic in the worst case. In Section \ref{s:empiricalcost}, we show that the theoretical worst case is a poor predictor of the empirical runtime of iterative deletion. In this section, we examine the substantial gap between the mathematical properties of the iterative deletion algorithm and the linguistic properties of English syntax. While in principle any vertex may be pruned from a syntax tree, coherent english sentences require verbs and subjects. Coordinated English phrases must be joined with a conjunction. Prepositions cannot be removed from the start of an English prepositional phrase. ILPs consider an exponential number of possible sentence compressions, but in practice there is a much smaller set of fluid or coherent shortened sentences. \ahcomment{look at some deps which are never pruned in FA}
...
Our method might be extended with paraphrase operations in future work, following other approaches which blend abstractive and extractive summarization \cite{P17-1099}. However, deploying such abstractive  compression is a far-off goal: even minor semantic errors from abstractive methods will destroy user trust in an automated summary \cite{Handler17,Zhang}.
...
Prior work does not specify all features in the structured perceptron model. We implement every feature discussed in the published work, except in cases where the feature or constraint assumes a syntactic formalism which is not compatible with UD.
 ...
 \ahcomment{TODO: table with final parameters of the neural network} 
