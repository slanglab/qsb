...
to be is all tokens in $V$ which occur to the left of  $T(v)$ in $s$; and define $V_R \subseteq V$ to be all tokens in $V$ which fall to the right of $T(v)$ in $s$. We also define a linearization function, $\ell(  \{v_1, v_2 ... v_n \} )$, which returns all vertexes in some set, linearized in their order in $s$.\footnote{$\ell$ is needed because sets do not have order} 
