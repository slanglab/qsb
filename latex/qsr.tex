...
We define a markup function $m$ which converts the state of the compression system into a sequence of symbols $\bm{x}$, called \textbf{the markup}. The state of the compression system  (\S\ref{s:system}) is a tuple $(V, v|B)$, where $V$ is a set of vertexes, and $v$ is a single vertex in the first position of $B$, the buffer. Intuitively, the markup ``shows'' the LSTM what the sentence would ``look'' like if $T(v)$ were to be inserted into or pruned from $V$. 

More formally, we again use $T(v)$ to denote all vertexes in the subtree governed by $v$ in $s$. We then define $V_L \subseteq V$
to be is all $v \in V$ which occur to the left of $T(v)$ in $s$. We also define $V_R \subseteq V$ identically, but tokens must fall to the right of $T(v)$. We also define a linearization function, $\ell(  \{v_1, v_2 ... v_n \} )$, which returns all vertexes in some set, linearized in their order in $s$.\footnote{$\ell$ is needed because sets do not have order} 

The markup is a sequence, $m(V, v|B)$ = [SOS, $\ell(V_L)$, BracketTag, $\ell(T(v))$, BracketTag, $\ell(V_R)$, EOS], where SOS and EOS are out of vocabulary symbols denoting the start and end of the sequence. 

Each BracketTag has a more complex structure formed by concatenating three different markers: (1) a categorical marker indicating the dependency type governing $T(v)$, (2) a binary marker indicating the type of the proposed operation (i.e \textsc{Prune} or \textsc{Insert}) and (3) a binary marker indicating if the bracket comes before or after $T(v)$ in the markup. For instance, in Figure \ref{f:example}, \ahcomment{TODO rewrite} the BracketTag \textit{nmod-I-L} indicates that (1) $T(v)$ is governed by an \rdep{nmod} relation, (2) the proposed operation is \textit{Insert} and (3) the marker symbol comes to the left of $T(v)$ in the sequence. 
...
We note that it is only possible to $\textsc{Prune}$ $T(v)$ if $v \in V$, and only possible to \textsc{Insert} $T(v)$ if $v \notin V$. Therefore, the marker (2) indicating the type of the proposed operation is fully determined by $(V, v|B)$. For \textsc{Prune} operations, $T(v) \subseteq V$; for \textsc{Insert} operations, $T(v) \nsubseteq V$.
