...
The markup is the deterministic (rather than learned) output of a markup function $\bm{x}=m(V,v,o,s)$, which inserts three sets of matched, inline bracket symbols into $s$, the original sequence of tokens in the sentence. The bracket symbols represent the following information:
...
\ahcomment{english here} In addition to the three inline bracket symbols, the markup also includes special indicator tags which are concatenated to all tokens $t_i \in s \setminus V $. (These indicator tags are denoted with strikethrough text in Figure \ref{f:example}.) 
