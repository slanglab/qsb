...
We also experiment with learning acceptance decisions with a standard LSTM classifier, using a common max-pooling architecture and standard neural framework \cite{D17-1070,Gardner2017AllenNLP}. Our classifier maintains four vocabulary embeddings matrixes, corresponding to the disjoint subsets $V = C \cup \neg C \cup P \cup \{v_i\}$. We form each LSTM input vector $x_t$ by looking up the appropriate embedding for $v_t \in V$, depending on the state of the compression system at timestep $i$.
