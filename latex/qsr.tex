...
We use the training set from a large corpus of sentence--compression pairs \cite{filippova2013overcoming} to define a set of 8 million training instances of the form $(\bm{x}_j, y_j)$, where $\bm{x}_j$ is the markup at timestep $j$ on some oracle path, and $y_j \in \{0,1\}$ is the binary oracle choice at timestep $j$.
