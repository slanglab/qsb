...
In this work, we present a new transition-based sentence compression system. We formally describe the system (\S\ref{s:formal}), present a method for generating oracle path compressions using a standard dataset (\S\ref{s:oracle}), and then detail a computational model of oracle compression (\S\ref{s:modeling}).
...
We define four principal operations. The operation \textbf{\textsc{Prune}} removes all vertexes in $T(v)$ from $V$, while removing $v$ from the buffer. The operation \textbf{\textsc{Insert}} adds $T(v)$ into $V$, while also removing $v$ from the buffer. The compression system can also execute the operations \textbf{\textsc{NoPrune}} and \textbf{\textsc{NoInsert}}, which remove $v$ from $B$ without inserting or pruning the subtree $T(v)$. 
...
We define a markup function $m(V,v,o)=\bm{x}$ to generate the sequence of input symbols for the LSTM model. The markup function is meant to ``show'' the model what a compressed sentence would ``look'' like if it were to execute the operation $o \in \{\textsc{Prune}, \textsc{Insert}\}$. For instance, for insert operations, the markup ``shows'' what the resulting compression would be, if the subtree $T(v)$ were copied over into $V$. More concretely, the markup $\bm{x}$ is the symbol sequence [SOS, $V_L$, $\langle$BracketTag{\small $\rangle$}, $T(v)$, $\langle /$BracketTag$\rangle$, $V_R$, EOS], where:
