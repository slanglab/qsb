...
We evaluate the theoretical gains from \textsc{vertex addition}  (Table \ref{t:algos}) by measuring the empirical latency of each compression method (in milliseconds per sentence) for a sample of $N=300,000$ sentences. We observe that runtimes are log-normally distributed, and thus report latency as the sample geometric mean. \textsc{ilp} and \textsc{vertex addition}$_{LR}$ share edge feature extraction code to support to fair comparison. We test \textsc{vertex addition}$_{NN}$ using a CPU, to determine performance in settings without GPUs. The appendix further details latency and SLOR experiments.
...
\caption{Density plot of log transformed latencies for \textsc{Vertex Addition}$_{LR}$ (left) and \textsc{ilp} (right). Theoretical gains (Table \ref{t:algos}) create real speedups. \textsc{Vertex Addition}$_{LR}$ also has lower variance than the \textsc{ilp}.}
\label{f:times}
\end{figure}
