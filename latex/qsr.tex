...
We define a markup function, $m: (v,V,s) \rightarrow \bm{x}$, where $v$ is a vertex in the original sentence $s$, and $V$ is the set of vertexes in the state tuple at some step $j$ during compression. The output of the function, $\bm{x} = s_1, s_2, s_3 ... s_K$ is a sequence of symbols called \textbf{the markup}. 

Intuitively, the markup ``shows'' the LSTM what the sentence would ``look'' like if $T(v)$ were to be inserted into or pruned from the sentence. More formally, we use $T(v)$ to denote all vertexes in the subtree governed by $v$ in $s$. We then define $V_L \subseteq V$
to be is all tokens in $V$ which occur to the left of  $T(v)$ in $s$; and define $V_R \subseteq V$ to be all tokens in $V$ which fall to the right of the subtree. We also define a linearization function, $\ell(  \{v_1, v_2 ... v_n \}, s)$, which returns all vertexes in some set $\{v_1, v_2 ... v_n \}$, linearized in their order in $s$.\footnote{$\ell$ is needed because sets do not have order} 
