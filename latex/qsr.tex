...
Query terms only {\small (lower bound)} & 0.384  & a   \\
Supervised ILP  &  0.854   & a        \\
\textbf{Transition-based deletion} &  \textbf{0.875}  & a   \\
\end{tabular}
\caption{The token-level F1 score for our transition-based deletion approach to length and lexically constrained sentence compression, along with the F1 score for our reimplementation of a traditional ILP-based approach \cite{filippova2013overcoming}. The difference between the methods is statistically significant. Selecting only query terms for a compression achieves an F1 = 0.384, which is the lower bound for this task.}
\label{t:results}
\end{table}
...
We use the automated SLOR metric \cite{lau2015unsupervised} to check the readability of compressions. Prior work shows that this metric correlates with human readability judgements for the compression task \cite{kannConl}. SLOR normalizes the probability of a token sequence assigned from a language model, by adjusting for both the probability of the individual unigrams in the sentence and for the sentence length.\footnote{Longer sentences are always less probable than shorter sentences; rarer words make a sequence less probable.} The appendix contains additional details of our implementation of SLOR. Our transition-based iterative deletion technique achieves a test-time SLOR score of 0.906; the ILP achieves a SLOR score of 0.876. This suggests that each method returns similarly readable compressions.
