...
In this work, we present an alternative method for shortening sentences under lexical and length constraints. Our method, iterative deletion, shortens a sentence over $M$ time steps, by pruning $M$ subtrees from a dependency parse, one after another. This method recalls early solutions to the compression problem, which also removed components of sentences. \ahcomment{CITE: K and M, Jing  and McKowen} 

The benefits of iterative deletion include (1) easily making use of new and existing forms of supervision for the sentence compression task (2) easily acc

Like ILP-based methods, iterative deletion approaches can trivially accommodate lexical and length constraints: subtrees which contain query terms are never deleted, and pruning stops only when a compression satisfies a given character budget. 

The chief benefit of iterative deletion is that it occurs lower computational costs than ILPs, while making use of the 

In this work, we say that iterative deletion methods incur one unit of computational cost each time they evaluate a subtree for deletion. If a dependency parse contains $V$ vertexes and each is evaluated for possible deletion, this incurs a cost of $V$ units. In the worst case, an iterative deletion method prunes $V$ singleton subtrees, consisting of single vertexes, starting from the leaves of the original dependency parse and proceeding to the root of the original dependency parse. If all vertexes are evaluated at all time steps, then pruning the tree requires at most $\sum_{i = 0}^M V - i = O(V^2)$ operations. 


There are two free parameters in the iterative deletion approach: (1) how to decide if a particular subtree may be deleted and (2) what order should subtrees be considered for deletion. In this study, we consider varying answers to each of these two questions.
