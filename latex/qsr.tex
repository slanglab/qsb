...
We check the theoretical gains from \textsc{vertex addition}  (Table \ref{t:algos}) by measuring the empirical mean latency of each compression method, in milliseconds per sentence. We use the same edge feature extraction code for \textsc{ilp} and \textsc{vertex addition}$_{LR}$ to ensure each method incurs similar delay for this step. We measure latency of \textsc{vertex addition}$_{NN}$ using a CPU, to test if the method is practical in settings without GPUs. The appendix further details measurement of latency and SLOR.
...
\end{tabular}
\caption{Test results for constrained compression. Because observed runtimes are log normal, $^*$latency is reported as the geometric mean (in milliseconds per sentence), across 300,000 sentences. \textsc{vertex addition}$_{LR}$ achieves higher F1 than the \textsc{ilp}. The method is also \exact times faster. Differences between all scores for \textsc{vertex addition}$_{LR}$ and \textsc{ilp} are significant {\tiny $(p < .01)$}. {\textsc{vertex addition}$_{NN}$} runs slowly on a CPU.}
\label{t:results}
\end{table}
...
\caption{Density plot of log transformed latencies for \textsc{Vertex Addition}$_{LR}$ (left) and \textsc{ilp} (right). Theoretical gains (Table \ref{t:algos}) enable real speedups. Our method also has more consistent runtimes (lower variance) than the \textsc{ilp}.}
\label{f:times}
\end{figure}
