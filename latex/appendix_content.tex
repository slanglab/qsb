...
When comparing sentence compression systems, it is important to ensure that all approaches use the same compression ratio \cite{napoles2011evaluating}. Following \citet{filippova2015sentence}, we define the compression ratio as the character length of the compression divided by the character length of the sentence. We present test set compression ratios for all methods in Table \ref{t:cr}. Because ratios are similar, our comparison is appropriate.
...
\textsc{ilp} &  0.407 \\
\textsc{ablated} & 0.398 \\
\textsc{vertex addition} &  0.404  \\
\textsc{vertex addition}_{NN} &  0.405  \\ \midrule
Train  & 0.383 \\
Test   & 0.412 \\
\end{tabular}
\caption{Mean test time compression ratios (CR) for all techniques. We also show mean ratios for gold compressions across the train and test sets.}\label{t:cr}.
\end{table}
