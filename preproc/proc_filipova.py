...
The data from Google comes in a very annoying format. This file basically just translates it to a cleaner format

It runs CoreNLP annotation and adds some metadata like the token indexes of the compression
...
'''these are three helpers that deal w/ getting data out of an annoying google format into an easier format'''
    ...
    with open(sys.argv[1].replace(".jsonl", "") + ".source", "w") as of_s:
        for lno, ln in enumerate(inf):
            ...
            try:
                ...
                graph = [{"parent_id":k["parent_id"], "child_id":k["child_id"]} for k in ln["graph"]["edge"]]
                ...
                for sno, s in enumerate(source_toks):
                    ...
                    index = sno + 1
                    id2index[id_] = index
                id2index[-1] = 0 # -1 in fillipova's tokenization == 0 aka root for stanfordcorenlp

                graph_nodes_as_indexes = [list(graph_node_to_token_indexes(_)) for _ in ln["graph"]["node"]]
                ...
                for c in compression:
                    ...
                    child_node_in_graph = [_ for _ in graph_nodes_as_indexes if id2index[c["child_id"]] in _]
                ...
                print "compression references a tok id not in source {}-{}-{}".format(e, lno, sys.argv[1])
