...
Our compression approach decides to add or not add some vertex $v_i$ at each timestep. We learn such decisions using a corpus of tuples $(S,Q,b,C_g)$, where $C_g$ is some known-good compression of $S$ respecting $Q$ and $b$ (\S\ref{s:intro}). Given $(S,Q,b,C_g)$, we can construct an oracle compression path by initializing $C_0$ with $Q$, adopting policy $\pi$ and choosing to add $v_i$ to $C_i$ iff $v_i \in C_g$. We then use oracle paths to train a model of oracle inclusion decisions, $p(y_i  = 1 | v_i, C_i, F_i, S_i)$. We discuss our model in (\S\ref{s:transition}). Our oracle procedure can reconstruct all compressions in a large, standard corpus \cite{filippova2013overcoming}. 
