...
We initialize $C_0=Q$ and $F_0 = V \setminus Q$. Then, at each timestep, we decide if some vertex $v_i \in F_i$ should be added to $C_i$. We select $v_i$ from $F_i$ according to some policy $\pi$, denoted $v_i = \pi(F_i)$.  We continue adding vertexes until either the frontier is empty or $\ell(C)$ is longer than the length constraint. 
...
We use a model of oracle decisions to decide if $v_i$ should be added to $C_i$ at test time.  We model each oracle decision $y_i \in \{0,1\}$ with binary logistic regression, using features which are similar to prior approaches to supervised compression \cite{filippova2013overcoming,Filippova2015FastKS}.  
