...
\textbf{Interaction features} are formed by crossing all local and global features with the dependency type of the edge $(u,v_i)$. Such features are only used in cases where $u \in C_i$.
...
We introduce a new additive method for sentence compression under lexical and length constraints. Our approach is much \speedup faster than ILP-based techniques, and better reconstructs known-good constrained compressions. Our low-latency method can be used in interactive search systems, which require rapid, query-oriented compressions.
