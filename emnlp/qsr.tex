...
We developed our compression system empirically: for all compressions in a large, standard corpus \cite{filippova2013overcoming} there exists an oracle path of at most $|V|$ operations which can fully reconstruct the gold compression.

\ahcomment{need a diagram}

\subsection{Modeling}\label{s:modeling}

We use a model of oracle decisions to decide if $v_i$ should be added to $C_i$ at test time.  
