...
We show that this additive approach can construct constrained compressions (\S\ref{s:formal}) in linear time (\S\ref{s:costs}), making it well-suited to interactive search applications (Figure \ref{f:qf}) which must provide rapid feedback during browsing.

\subsection{Formal Description}\label{s:formal}

Our additive approach builds a compression $C$ over at most $|V|$ timesteps by maintaining a state
...
We initialize $C$ with $Q$ and initialize $F = V \setminus Q$. Then, at each timestep, we decide if some vertex $v_i \in F_i$ should be added to $C_i$. We continue adding vertexes until either the frontier is empty or $\ell(C)$ is longer than the length constraint. 
...
\ahcomment{In describing this, the key point is that there is info about the edges you can use.}
