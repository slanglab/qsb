...
\begin{abstract}
Traditional approaches to extractive sentence compression seek to reduce the length of a sentence, while retaining the most ``important'' information from the source. But query-focused applications such as document search engines or exploratory search interfaces place additional lexical, length and latency requirements on compression systems. This study introduces a new transition-based technique which accommodates such requirements, by building up a compression in linear time. We show that this approach is much faster than ILP-based techniques, crucial for interactive search applications. Our method also achieves superior performance in reconstructing known-good shortenings.
\end{abstract}
...
\ahcomment{bottom up presumes query is at leaves. say additive}
...
The chief advantage of this method is that can construct constrained compressions (\S\ref{s:formal}) in linear time (\S\ref{s:costs}), making it well-suited to interactive search applications (e.g.~Figure \ref{f:qf}) which must provide rapid, query-focused snippets during browsing.
...
We initialize $C$ with $Q$ and initialize $F$ with $V \setminus Q$. Then, at each timestep, we decide to either add or  not add some vertex $v_i \in F_i$ to $C_i$. After the decision, we remove $v_i$ from $F_i$. 

We continue adding vertexes until either $F$ is empty or the linearized compression, $\ell(C)$, is longer than the length constraint.\footnote{We linearize tokens in their original sentence order. \ahcomment{explain}}

We select $v_i$ from $F_i$ according to some policy $\pi$, denoted $v_i = \pi(F_i)$. In this work we use a rule-based policy $\pi$: neighbors of $C$ are selected from $F$ before non-neighbors. If more than one neighboring or non-neighboring vertex is available in $F$ we proceed in left-to-right order. Learned policies \cite{sutton2018reinforcement} might improve performance, but are left for future work. 

At training time, we use an oracle to decide if $v_i$ should be added to $C_i$. If $v_i$ is included in a gold compression, the vertex is added to $C_i$. At runtime, we use a model of oracle decisions $p(v_i|C_i)$ to determine if a vertex should be included. This method of determining paths based on oracle decisions can reconstruct all shortenings in a large, standard compression corpus \cite{filippova2013overcoming}.

\ahcomment{need a diagram}

\subsection{Modeling}\label{s:modeling}

\ahcomment{In describing this, the key point is that there is info about the edges you can use. Cite prior work}

\ahcomment{there is also global info you can use}

At test time, we use a model of oracle decisions to decide if $v_i$ should be added to $C_i$.  We model each oracle decision $y_i \in \{0,1\}$ with binary logistic regression.

Prior approaches to supervised, dependency-based sentence compression 

using features which are similar to prior approaches to supervised compression \cite{filippova2013overcoming,Filippova2015FastKS}.  
