...
\newcommand{\hideforemnlp}[1]{}

% for arxiv
%\newcommand{\emnlponly}[1]{}
%\newcommand{\hideforemnlp}[1]{#1}
...
During initialization, we set $C_0 \gets Q$ and $P_0 \gets S \setminus Q$. Then, at each timestep, we pop some candidate $v_i =h(P_i)$ from the head of $P_i$ and evaluate $v_i$ for inclusion in $C_i$. (Neighbors of $C_i$ in $P_i$ get higher priority than non-neighbors; we break ties in left-to-right order, by sentence position). If we accept $v_i$, then $C_{i + 1} \gets C_i \cup v_i$; if not, $C_{i + 1} \gets C_i$. 
(Acceptance decisions are detailed in \S\ref{s:transition}.) % BTO aug30: it was confusing to use ``we'' for both `we'=algorithm and `we'=writer
We continue adding vertexes to $C$ until either $P_i$ is empty or $C_i$ is $b$ characters long.\footnote{We linearize $C$ by left-to-right vertex position in $S$, common for compression in English \cite{filippova2013overcoming}.} The appendix includes a formal algorithm. 
...
Thanks to Javier Burroni and Nick Eubank for suggesting ways to optimize and measure performance of Python code. Thanks to Jeffrey Flanigan, Katie Keith and the UMass NLP reading group for feedback.
%helping us clarify our work. 
This work was partially supported by IIS-1814955.

\emnlponly{
\section*{Appendix}
The appendix is available with the arXiv version of this paper:
\url{https://arxiv.org/abs/1904.09051}
}
