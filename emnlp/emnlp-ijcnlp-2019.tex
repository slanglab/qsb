...
Our method also assumes access to a reliable, dependency parse, and ignores any latency penalties incurred from parsing. Both assumptions are unreasonable. Like other NLP tools, dependency parsers often perform poorly on out of domain text \cite{bamman2017natural}, and users looking to quickly investigate a new corpus (e.g.\ browsing collection of digitized reports) might not wish to wait for a parser. Faster approaches based on low-latency part-of-speech tagging offer an exciting possibility for additional study. 

Our approach also assumes that a user already knows a reasonable $b$ and reasonable $Q$ for a given sentence $S$.\footnote{Recall that we simulate $b$ and $Q$ based on the well-formed shortening $C_g$, see \S\ref{s:constrained}.} However, in some cases, there will not be a well-formed shortening for the triple $(S, Q, b)$. For instance, there is no well-formed shortening for the toy sentence ``Kennedy kept running'' for $Q$=``Kennedy'' and $b$=15. While ``Kennedy kept'' and ``Kennedy running'' each formally respect the query constraints, neither is a well-formed sentence. Because real applications will need to accommodate such cases, we look forward to investigating which $(Q,S,b)$ triples will never return well-formed compressions. This work 

Finally, some shortened sentences will modify the meaning of a sentence. We ignore this important complication in this initial study. However, in the future, we also hope to apply ongoing research into textual entailment \cite{snli_bowman,Pavlick2016SoCalledNA,linzencompression} towards better approaches to the compression task.
