...
def len_current_compression(current_compression, sentence):
    ...
    return np.sum(sentence["lengths"][current_compression]) + len(current_compression) - 1


def pick_l2r_connected(frontier, current_compression, sentence):
    connected = get_connected3(sentence, frontier, current_compression)
    
    # min index gets the first element, in L to R order
    if len(connected) > 0:
        return min(connected)
    else:
        return np.nonzero(frontier)[0][0] + 1 
...
def preproc(sentence, dependencies="basicDependencies"):
    ...
    indexes = set()
    lens = np.zeros(len(sentence["tokens"]), dtype=np.int32)
    ...
    ix2feats_included = {}
    G = np.zeros((len(sentence["tokens"]), len(sentence["tokens"])))
...
def runtime_path(sentence, frontier_selector, clf, vectorizer, decider=make_decision_lr,  marginal=None):
    ...
    cs = sentence["q"]
    current_compression2 = [(i - 1) for i in sentence["q"]]

    frontier2 = np.zeros(len(sentence["tokens"]), dtype=np.bool) + 1
    frontier2[current_compression2] = 0

    lt = len_current_compression(current_compression2, sentence)

    depths = sentence["depths"]

    while np.sum(frontier2) > 0 and lt < sentence["r"]:
        
        vertex = frontier_selector(frontier=frontier2,
                                   current_compression=current_compression2,
                                   sentence=sentence)
        wouldbe = lt + 1 + sentence["lengths"][vertex - 1]

        if wouldbe <= sentence["r"]:
            y = decider(vertex=vertex,
                        sentence=sentence,
                        depths=depths,
                        current_compression=cs,
            ...
            if y == 1:
                current_compression2.append(vertex - 1)
                cs.add(vertex)
                lt = wouldbe

        #frontier.remove(vertex)

        frontier2[vertex - 1] = 0
...
def get_adjacent3(sentence, current_compression, frontier2):
    return set(np.nonzero(frontier2 * np.sum(sentence["neighbors_mx"][current_compression], axis=0))[0] + 1)

def get_adjacent2(sentence, current_compression):
    out = set()
...
def get_connected2(sentence, frontier, current_compression):
    ...
    out = get_adjacent2(sentence, current_compression)
    out &= frontier # b/c intersects w/ frontier no need to exclude current compression
    return out

def get_connected3(sentence, frontier2, current_compression_l):
    '''get vertexes in frontier that are conntected to current_compression'''
    return np.nonzero(frontier2 * np.sum(sentence["neighbors_mx"][current_compression_l], axis=0))[0] + 1
...
def get_global_feats(sentence, feats, vertex, current_compression, lc):
    ...
    len_tok = sentence["lengths"][vertex - 1]
